...
const PostDetailPage = () => {
  ...
  const [post, setPost] = useState([]);
    const postData = async ()=>{       
      try {
        const data = await getSinglePost(params.post);
        setPost(data);
    ...
    <div className="singlePage">
      <div className="p-6 bg-white border border-gray-300 rounded-md">
        <div className="sectionTitle">
          <Badge variant="destructive" onClick={() => navigate(`/category/${post.category_url}`)} className="cursor-pointer">{post.category_name}</Badge>
          <h1 className="text-4xl font-bold my-8">{post.title}</h1>
          <Separator className="mb-6" />
            <div className="flex justify-between gap-4">
              <div className="flex justify-start items-start gap-3">
                <Link to={`/author/${post.user_url}`} className="w-10 h-10 rounded-full overflow-hidden">
                    <img src={`https://ik.imagekit.io/devamitjha/react/${post.featured_image}`} alt="author-name" className="w-full h-full object-cover overflow-hidden"/>
                </Link>
        ...
        <AspectRatio ratio={16 / 9} className="bg-muted rounded-lg mt-10 mb-6">
          <img src={`https://ik.imagekit.io/devamitjha/react/${post.featured_image}`} alt={post.title} className="h-full w-full rounded-lg object-cover overflow-hidden"/>
        </AspectRatio>
          ...
          <div className="flex justify-between items-center gap-4">
            <div className="flex justify-start items-start gap-3 w-full">               
              ...
              <div className="flex items-start flex-col justify-start space-y-1">                
                  ...
                  <h6 className="text-sm text-gray-500"><Link to="/author/name" className="text-red-600">{post.user_name}</Link></h6>
                  <div className="text-sm text-gray-500">{post?.bio || "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."}</div>
              </div> 
            </div>
